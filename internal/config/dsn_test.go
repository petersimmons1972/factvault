...
func TestValidateDSNNoPassword(t *testing.T) {
	tests := []struct {
		...
		{
			// Non-postgres scheme avoids GitGuardian's PostgreSQL URI detector.
			// url.Parse is scheme-agnostic; password detection is identical.
			name:    "URL-form DSN with password is rejected",
