...
public_dir="${FACTVAULT_AUTH_PUBLIC_DIR:-$auth_dir}"
public_key="${FACTVAULT_JWT_PUBLIC_KEY:-$public_dir/factvault-jwt-public.pem}"
...
if [ "${FACTVAULT_BOOTSTRAP_AUTH:-1}" != "0" ]; then
	...
	if [ "${FACTVAULT_EXPORT_PRIVATE_KEY:-0}" = "1" ]; then
